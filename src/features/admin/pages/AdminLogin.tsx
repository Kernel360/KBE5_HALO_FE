  ...
  <Card variant="login" className="w-[480px] p-0 flex flex-col gap-0">
    ...
    <div className="pt-10 pb-6 border-b border-gray-100 bg-gradient-to-b from-indigo-50 to-white rounded-t-2xl flex flex-col items-center gap-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-gray-500 text-lg font-semibold tracking-wide">관리자 포털</span>
      </div>
      <div className="text-gray-400 text-xs mt-1">HaloCare 운영을 위한 관리자 전용 공간입니다.</div>
    </div>
    {/* 폼 영역 */}
  ...
  return (
    <Fragment>
      <div className="w-full min-h-screen bg-gray-50 flex justify-center items-center px-2 py-8">
        <div className="flex flex-col-reverse md:flex-row gap-8 md:gap-12 items-center w-full max-w-4xl">
          ...
          >
            <div className="flex flex-col gap-2 px-6 md:px-10 pt-8 md:pt-10 pb-4">
              <div className="flex items-center gap-2">
                <span className="w-8 h-8 flex items-center justify-center bg-indigo-100 rounded-full text-indigo-600 text-xl font-bold">★</span>
                <span className="text-indigo-700 text-lg md:text-xl font-extrabold font-['Inter'] tracking-tight">HALO 관리자 주요 기능</span>
              </div>
              <div className="text-gray-600 text-sm md:text-base font-semibold font-['Inter'] mt-1">무엇을 할 수 있나요?</div>
            </div>
            <CardContent className="flex flex-col gap-3 p-4 md:p-8 pt-2">
              {[
                { title: "관리자 계정 관리", desc: "계정 추가, 수정, 삭제 및 권한 관리" },
                { title: "고객/매니저 정보 관리", desc: "고객·매니저 정보, 계약, 평가, 예약 현황 관리" },
                { title: "문의/예약/배너/공지 관리", desc: "문의, 예약, 배너, 공지/이벤트 게시글 관리" },
              ].map((item, idx) => (
                <div
                  key={item.title}
                ...
                >
                  <span className="w-8 h-8 flex items-center justify-center bg-indigo-100 rounded-full text-indigo-600 text-lg font-bold">{['①','②','③'][idx]}</span>
                  <div>
                    <div className="text-gray-900 font-bold text-base md:text-base">{item.title}</div>
                  ...
                  {openFeatureIndex === idx && (
                    <div
                      className={
                        `absolute z-20 w-[90vw] max-w-xs left-1/2 -translate-x-1/2 top-full mt-2
                    ...
                    >
                      <div className="relative bg-white border border-indigo-200 rounded-lg shadow-lg px-3 md:px-4 py-2 md:py-3 text-xs md:text-sm text-gray-700 w-full animate-fade-in">
                        ...
                        <div
                          className={
                            `absolute left-1/2 -translate-x-1/2 -top-2 w-0 h-0 border-x-8 border-x-transparent border-b-8 border-b-white border-t-0
        ...
        <ErrorToast open={toast.open} message={toast.message} onClose={() => setToast({ open: false, message: "" })} />
      </div>
    </Fragment>
  );
};