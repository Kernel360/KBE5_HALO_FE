...
export const AdminAccounts = () => {
  ...
  const [targetAdminId, setTargetAdminId] = useState<string | number | null>(null);
  ...
  const fetchData = async (paramsOverride?: Partial<ReturnType<typeof getSearchParams>>) => {
    setLoading(true);
    try {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => <AccountStatusBadge status={row.status === '활성' ? 'ACTIVE' : row.status === '비활성' ? 'SUSPENDED' : row.status} />,
    },
    {
      key: 'action',
      header: '삭제',
          ...
          <TableSection title="계정 정보" total={adminData.length}>
            ...
            <div className="hidden md:block">
              <AdminTable
                ...
                onRowClick={row => navigate(`/admin/accounts/${row.adminId}/edit`)}
              />
            ...
            <div className="block md:hidden">
              {adminData.length === 0 ? (
                <div className="text-center text-gray-400 py-8">등록된 관리자가 없습니다.</div>
              ) : (
                <div className="flex flex-col gap-3">
                  {adminData.map(row => (
                    <div
                      ...
                      onClick={() => navigate(`/admin/accounts/${row.adminId}/edit`)}
                    >
